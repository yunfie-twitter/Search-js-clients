...
const Settings: React.FC = () => {
  ...
  const {
    themeMode, setThemeMode, language, setLanguage, saveHistory, setSaveHistory,
  ...
  const handleShowSyncQr = async () => {
    ...
    try {
      ...
      setShowSyncQr(true);
      triggerHaptic();
    } catch (err) { setSnackMsg(language === 'ja' ? 'QRコードの生成に失敗しました' : 'Failed to generate QR'); }
  };
  ...
  const handleStartScan = async () => {
    ...
    try {
      ...
      if (videoRef.current) { videoRef.current.srcObject = stream; videoRef.current.play(); requestAnimationFrame(scanFrame); }
    } catch (err) { setShowQrScanner(false); setSnackMsg(language === 'ja' ? 'カメラへのアクセスが拒否されました' : 'Camera access denied'); }
  };
  ...
  const scrollToSection = (id: SectionId) => {
    setActiveSection(id);
    if (isMobile) document.getElementById(`section-${id}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  };
    ...
    <>
      ...
      <div id="section-privacy">
        ...
        <Paper elevation={0} sx={{ borderRadius: '16px', border: '1px solid', borderColor: 'divider', overflow: 'hidden' }}>
          <List sx={{ py: 0 }}><SwitchItem icon={<HistoryIcon />} primary={t.saveHistory} checked={saveHistory} onChange={setSaveHistory} /></List>
        </Paper>
      </div>
      <div id="section-sync">
        ...
        <Paper elevation={0} sx={{ borderRadius: '16px', border: '1px solid', borderColor: 'divider', overflow: 'hidden' }}>
          <Box sx={{ p: 2, textAlign: 'center', bgcolor: 'action.hover' }}>
             <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>{language === 'ja' ? 'QRコードを使用して他の端末とペアリングし、設定を同期します。' : 'Pair with other devices using QR code to sync settings.'}</Typography>
             <Stack direction="row" spacing={2} justifyContent="center">
               <Button variant="contained" startIcon={<QrCodeIcon />} onClick={handleShowSyncQr} sx={{ borderRadius: '12px', textTransform: 'none' }}>{language === 'ja' ? '同期QRを表示' : 'Show Sync QR'}</Button>
               <Button variant="outlined" startIcon={<ScanIcon />} onClick={handleStartScan} sx={{ borderRadius: '12px', textTransform: 'none' }}>{language === 'ja' ? 'スキャン' : 'Pair Scan'}</Button>
             </Stack>
          </Box>
          <Divider />
          <List sx={{ py: 0 }}>
          ...
          <Box sx={{ p: 2 }}>
            ...
            <Stack spacing={2}>
              <TextField label="Group ID" value={syncGroupId} onChange={(e) => setSyncGroupId(e.target.value.toUpperCase())} fullWidth size="small" InputProps={{ startAdornment: <InputAdornment position="start"><GroupIcon fontSize="small" /></InputAdornment>, sx: { borderRadius: '10px' } }} />
              <Box>
      ...
      <div id="section-about">
        ...
        <Paper elevation={0} sx={{ borderRadius: '16px', border: '1px solid', borderColor: 'divider', overflow: 'hidden' }}>
          <List sx={{ py: 0 }}>
            <ListItem sx={{ py: 1.5, cursor: 'pointer' }} onClick={handleVersionTap}><ListItemIcon><InfoIcon /></ListItemIcon><ListItemText primary="Version" secondary="1.2.0" /><ChevronRightIcon /></ListItem>
            {expUnlocked && <><Divider /><ListItem button onClick={() => navigate('/labs')}><ListItemIcon><LabsIcon color="warning" /></ListItemIcon><ListItemText primary="Experimental Settings" /><ChevronRightIcon /></ListItem></>}
          </List>
        </Paper>
      </div>
    </>
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100dvh', width: '100%', overflow: 'hidden', backgroundColor: 'background.default' }}>
      <PageHeader title={t.settings} />
      <Box sx={{ flexGrow: 1, overflow: 'hidden', display: 'flex' }}>
        {!isMobile && (
          <Box sx={{ width: 280, borderRight: '1px solid', borderColor: 'divider', bgcolor: 'background.paper', p: 2, display: 'flex', flexDirection: 'column', gap: 1 }}>
            <Typography variant="overline" sx={{ px: 2, color: 'text.secondary', fontWeight: 700 }}>Category</Typography>
            <List sx={{ p: 0 }}>
              {sidebarItems.map((item) => (
                <ListItem key={item.id} onClick={() => scrollToSection(item.id as SectionId)} sx={{ borderRadius: '12px', mb: 0.5, cursor: 'pointer', bgcolor: activeSection === item.id ? 'primary.main' : 'transparent', color: activeSection === item.id ? 'primary.contrastText' : 'text.primary', transition: 'all 0.2s ease', '&:hover': { bgcolor: activeSection === item.id ? 'primary.main' : 'action.hover' } }}>
                  <ListItemIcon sx={{ color: 'inherit', minWidth: 40 }}><item.Icon fontSize="small" /></ListItemIcon>
        ...
        <Box sx={{ flexGrow: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
          <PageTransition>
            <Container maxWidth={isMobile ? "sm" : "md"} sx={{ py: isMobile ? 2 : 4, px: isMobile ? 2 : 6, display: 'flex', flexDirection: 'column', gap: 2 }}>
              {!isMobile && <Typography variant="h4" sx={{ fontWeight: 800, mb: 2 }}>{sidebarItems.find(s => s.id === activeSection)?.label}</Typography>}
              {renderContent()}
              <Box sx={{ height: 100 }} />
            </Container>
            <BottomNavSpacer />
      ...
      <Dialog open={showDeleteConfirm} onClose={() => setShowDeleteConfirm(false)} PaperProps={{ sx: { borderRadius: '24px' } }}>
        <DialogTitle sx={{ fontWeight: 700 }}>全データの削除</DialogTitle>
        <DialogContent><DialogContentText>すべての設定と検索履歴が完全に削除されます。この操作は取り消せません。</DialogContentText></DialogContent>
        <DialogActions sx={{ p: 2 }}><Button onClick={() => setShowDeleteConfirm(false)}>Cancel</Button><Button onClick={() => { resetAllData(); window.location.reload(); }} variant="contained" color="error">Delete Everything</Button></DialogActions>
      </Dialog>
      <Dialog open={showSyncQr} onClose={() => setShowSyncQr(false)} PaperProps={{ sx: { borderRadius: '24px', p: 2 } }}>
        <DialogTitle sx={{ fontWeight: 800, textAlign: 'center' }}>同期ペアリング</DialogTitle>
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}><Box component="img" src={qrImageUrl} sx={{ width: '100%', maxWidth: 300, borderRadius: '16px' }} /><Typography variant="caption" color="text.secondary" textAlign="center">もう一方の端末でこのQRをスキャンして同期を開始します</Typography></DialogContent>
        <DialogActions><Button fullWidth onClick={() => setShowSyncQr(false)} variant="contained" sx={{ borderRadius: '12px' }}>Done</Button></DialogActions>
      </Dialog>
      <Dialog open={showQrScanner} onClose={stopScan} fullScreen PaperProps={{ sx: { bgcolor: '#000' } }}>
        <Box sx={{ position: 'relative', width: '100%', height: '100%', display: 'flex', flexDirection: 'column' }}>
          <Box sx={{ position: 'absolute', top: 'calc(env(safe-area-inset-top) + 16px)', left: 16, zIndex: 10 }}><Button onClick={stopScan} sx={{ color: '#fff', bgcolor: 'rgba(0,0,0,0.5)', borderRadius: '12px' }}>Cancel</Button></Box>
          <video ref={videoRef} style={{ width: '100%', height: '100%', objectFit: 'cover' }} playsInline /><canvas ref={canvasRef} style={{ display: 'none' }} />
          <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 250, height: 250, border: '2px solid #fff', borderRadius: '24px', boxShadow: '0 0 0 4000px rgba(0,0,0,0.5)' }}>
            <Box sx={{ position: 'absolute', top: -2, left: -2, width: 40, height: 40, borderLeft: '4px solid #fff', borderTop: '4px solid #fff', borderTopLeftRadius: '24px' }} /><Box sx={{ position: 'absolute', top: -2, right: -2, width: 40, height: 40, borderRight: '4px solid #fff', borderTop: '4px solid #fff', borderTopRightRadius: '24px' }} /><Box sx={{ position: 'absolute', bottom: -2, left: -2, width: 40, height: 40, borderLeft: '4px solid #fff', borderBottom: '4px solid #fff', borderBottomLeftRadius: '24px' }} /><Box sx={{ position: 'absolute', bottom: -2, right: -2, width: 40, height: 40, borderRight: '4px solid #fff', borderBottom: '4px solid #fff', borderBottomRightRadius: '24px' }} />
          </Box>
          <Box sx={{ position: 'absolute', bottom: 'calc(env(safe-area-inset-bottom) + 40px)', width: '100%', textAlign: 'center' }}><Typography sx={{ color: '#fff', fontWeight: 600 }}>QRコードを枠に合わせてください</Typography></Box>
        </Box>
      </Dialog>
      <Snackbar open={!!snackMsg} autoHideDuration={2000} onClose={() => setSnackMsg('')} message={snackMsg} anchorOrigin={{ vertical: 'bottom', horizontal: 'center' }} ContentProps={{ sx: { borderRadius: '12px' } }} />
    </Box>
  );
};
